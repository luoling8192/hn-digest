...
- Telegram messages start with one or two AI-selected topic tags and a short scanning card: one-sentence takeaway followed by two or three concrete highlights. The model receives previously used tags and their usage counts, reuses an accurate existing tag when possible, and creates a new tag only when needed. Generic fallback tags such as `其他` are rejected. The Telegraph page still contains the complete original article summary before the HN discussion summary.
